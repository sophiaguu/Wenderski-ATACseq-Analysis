...
# awk '{print NF}' DEpeaks/baselinevsknockout_ATACdiffpeaks.txt | uniq
...
awk '{if($26<=0.05) {print}}' DEpeaks/baselinevsknockout_ATACdiffpeaks.txt > DEpeaks/FDRfiltered.baselinevsknockout_ATACdiffpeaks.txt
...
grep -v '^#' DEpeaks/FDRfiltered.baselinevsknockout_ATACdiffpeaks.txt > DEpeaks/tmp.txt

    perl pos2bedmod.pl DEpeaks/tmp.txt > DEpeaks/tmp.bed

    sed 's/^/chr/' DEpeaks/tmp.bed > DEpeaks/FDRfiltered.baselinevsknockout_ATACdiffpeaks.bed
